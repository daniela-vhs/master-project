import pandas as pd
from dates import target_holidays, Tenor

holidays = target_holidays(2019, 2057)
...
with pd.ExcelFile("raw_data/vols.xlsx") as f:
    ...
    for strike in f.sheet_names:
        ...
        df["Tenor"] = df.Tenor.apply(lambda x: Tenor(f"{x}Y").tenor).astype("category")
        vol_data.append(df)

    vol_data = pd.concat(vol_data).set_index(["Date", "Tenor", "Strike"]).sort_index().reset_index()

vol_data = vol_data[~vol_data.Date.isin(holidays)]
vol_data.to_parquet("clean_data/vols.parquet", index=False)
